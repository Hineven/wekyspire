...
* **断神斩**系列是本游戏最高伤害单卡，但条件苛刻——仅能在牌库中冷却，在手中渡过回合则会快速衰败。
  * 分支：**强斩**系列，条件不太苛刻，但伤害也相对平凡一些。
* **风暴刀舞**系列是功能和伤害型卡牌，可以低费快速弃牌。
  * 分支：**优雅刀舞**系列依赖弃牌获得格挡和护盾。【未测试】
  * 分支：**连环刀舞**弃牌后抽等量牌。【未实现】
  * 分支：**完美花刀**能造成高额伤害并选牌丢弃。
* **回旋斩**系列可以快速低费从**牌库末**抽牌。
* **刀背打击**系列可以从右手边丢牌。【未实现】
* **呼吸**系列消耗性功能卡能让本回合内丢牌时抽牌。
  * 分支：**坚强呼吸**同时能获得格挡。
  * 分支：**柔韧呼吸**同时能获得护盾。
  * 分支：**完美呼吸**同时获得格挡和护盾。
* **飞刀**系列依赖丢弃两侧牌造成高额单次伤害。
  * 分支：**回旋飞刀**弃牌后抽牌进入两侧。
  * 分支：**完美飞刀**消耗两侧牌来从牌库中找牌抽取。【未测试】
* **藏锋**系列消耗性功能卡，造成高额伤害并获得滞气。
* 深入：**开刃**系列消耗性功能卡，即刻冷却手中刀法牌，回合结束时自动进入牌库。
* 深入：**斩灭**系列消耗性功能卡，下一次伤害必中且翻倍，回合结束时自动进入牌库。【未测试】
* 深入：**锻刀**系列消耗性功能卡，锻造（power up）一张手中刀法牌并丢弃，回合结束时自动进入牌库。【未实现】
* 咏唱：**养刀术**，系列持续提升牌库中刀组合卡牌的强度（power up）。【未测试】
  * 深入分支：**拔刀术**，反转斩系列卡牌特性，令其在手中冷却，在牌库中衰败，持续【2】回合。
...
* 咏唱：**以无胜有 / 以有胜无**，结束回合时，若没有/有许多手牌，获得高额格挡和护盾。【未实现】
  * 深入分支：**混元**，结束回合时，若没有或有多于六张手牌，获得高额格挡和护盾。【未实现】
