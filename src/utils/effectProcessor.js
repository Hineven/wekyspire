 ...
 * @param {Array} battleLogs - 战斗日志数组
 */
export function processStartOfTurnEffects(target, battleLogs) {
  // 处理燃烧效果
  if (target.effects['燃烧'] > 0) {
    ...
    battleLogs.push(`${target.name}被烧伤了，受到${damage}伤害！`);
    target.hp -= damage;
  ...
  if (target.effects['眩晕'] > 0) {
    ...
    battleLogs.push(`${target.name}处于眩晕状态，跳过回合！`);
 ...
 * @param {Array} battleLogs - 战斗日志数组
 */
export function processEndOfTurnEffects(target, battleLogs) {
  // 处理吸收效果
  if (target.effects['吸收'] > 0) {
    target.mana += target.effects['吸收'];
    battleLogs.push(`${target.name}通过吸收效果恢复了${target.effects['吸收']}点魏启！`);
  }
  ...
  if (target.effects['漏气'] > 0) {
    ...
    battleLogs.push(`${target.name}因漏气效果失去了${target.effects['漏气']}点魏启！`);
  }
  ...
  if (target.effects['中毒'] > 0) {
    ...
    battleLogs.push(`${target.name}受到中毒效果影响，失去了${damage}点生命！`);
  }
  ...
  if (target.effects['再生'] > 0) {
    ...
    battleLogs.push(`${target.name}通过再生效果恢复了${heal}点生命！`);
  }
  ...
  if (target.effects['超然'] > 0) {
    ...
    battleLogs.push(`${target.name}通过超然效果获得了${stacks}层集中！`);
  }
  ...
  if (target.effects['侵蚀'] > 0) {
    ...
    battleLogs.push(`${target.name}受到侵蚀效果影响，失去了${stacks}层集中！`);
  }
  ...
  if (target.effects['燃心'] > 0) {
    ...
    battleLogs.push(`${target.name}通过燃心效果获得了1层集中！`);
  }
  ...
  if (target.effects['成长'] > 0) {
    ...
    battleLogs.push(`${target.name}通过成长效果获得了${stacks}层力量！`);
  }
  ...
  if (target.effects['衰败'] > 0) {
    ...
    battleLogs.push(`${target.name}受到衰败效果影响，失去了${stacks}层力量！`);
  }
  ...
  if (target.effects['巩固'] > 0) {
    ...
    battleLogs.push(`${target.name}通过巩固效果获得了${stacks}层坚固！`);
  }
  ...
  if (target.effects['崩溃'] > 0) {
    ...
    battleLogs.push(`${target.name}受到崩溃效果影响，失去了${stacks}层坚固！`);
  }
  ...
  if (target.effects['魏宗圣体'] > 0) {
    ...
    battleLogs.push(`${target.name}通过魏宗圣体效果获得了${stacks}层集中、力量和坚固！`);
  }
  ...
  if (target.effects['解体'] > 0) {
    ...
    battleLogs.push(`${target.name}受到解体效果影响，失去了${stacks}层集中、力量和坚固！`);
  }
}
 ...
 * @param {Array} battleLogs - 战斗日志数组
 */
export function processSkillActivationEffects(target, battleLogs) {
  // 处理连发效果
  if (target.effects['连发'] > 0) {
    ...
    battleLogs.push(`${target.name}通过连发效果获得了1点行动力！`);
  }
}
 ...
 */
export function processDamageTakenEffects(target, damage, battleLogs) {
  ...
  if (target.effects['格挡'] > 0) {
    ...
    battleLogs.push(`${target.name}通过格挡效果将伤害减半！`);
  }
  
  // 处理闪避效果
  if (target.effects['闪避'] > 0) {
    finalDamage = 0;
    battleLogs.push(`${target.name}通过闪避效果完全回避了攻击！`);
  }
 ...
 * @param {Array} battleLogs - 战斗日志数组
 */
export function processDamageDealtEffects(target, damage, battleLogs) {
  // 处理暴怒效果
  if (target.effects['暴怒'] > 0) {
    ...
    battleLogs.push(`${target.name}通过暴怒效果获得了${stacks}层力量！`);
  }
  ...
  if (target.effects['执着'] > 0) {
    ...
    battleLogs.push(`${target.name}通过执着效果获得了${stacks}层集中！`);
  }
}
 ...
 * @returns {number} 处理后的伤害值
 */
export function processPostAttackEffects(attacker, target, damage, battleLogs) {
  let finalDamage = damage;
  ...
  if (attacker.effects['超频'] > 0) {
    ...
    if (Math.random() < 0.1) {
      finalDamage *= 2;
      battleLogs.push(`${attacker.name}的攻击触发了超频效果，伤害翻倍！`);
    }
  }
  ...
  if (target.effects['灼烧'] > 0) {
    ...
    if (Math.random() < 0.15) {
      ...
      battleLogs.push(`${target.name}受到灼烧效果影响，获得了1层燃烧！`);
    }
  }
