...
- **双 pass 渲染（用户定）**：stage 可带 `uiScene`——先渲染 3D 世界，清深度后再渲染
  UI pass（卡牌/按钮/牌库坟墓图标/资源点/查看器）。牌桌 UI 世界坐标在地板平面之下
...
- **战线轴 battleLine**（3D，站水平地板）：near (-50, FLOOR_Y, +22) → far (70, FLOOR_Y, -34)。
  槽位 t∈[0,1]：position = lerp + **lane 地面 XZ 法向偏移**（(dz,-dx) 向，lane+ = 屏幕左上/
  后排，lane- = 屏幕右下/前排；laneGap=16——偏 z 向的偏移在透视下才有足够屏幕分离，
  XY 法向会抬离地面，已废）；scale 只保留少量风格化夸张（1.05→0.9）。
  **前排（lane<0、z>0）单位的下半身会被手牌扇遮住**（UI 正直投影，世界俯角越大遮越多）——
  友军槽位一律放后排（瑞米 lane+0.9 主角右后方）；敌人前后排混合时点上半身/头部指定目标。
...
- **布局（五轮定框架、六轮定稿，用户定）**：**左墙（x=-95）= 月光墙**——分段盒子拼出的
  ...
  **墙体尺寸三铁律（六轮漏光调试实录，全部用数学验证过）**：
  ...
  ③**z 向覆盖**：光向 dz/dx≈0.106，房间远端漂移 ±25——现 z∈[-75,+125] 堵侧漏。
...
- **dungeon3D.js 其余**：水平地板（z -45..+95，左端止于左墙）+ 错位石板×10 + 裂板一对；
  扶壁×3 不对称；吊灯偏心（(-8,20,-20)，铁环+冷白烛焰）；**柱列不对称**：左列三根完好
  高低错落，右列前/后完好微倾、中柱断裂（残桩+斜茬顶+倒柱段+柱帽残块+小碎块簇）；
  碎石散点；全体石材 castShadow/receiveShadow（光影参与月光构图）。
- **幽火壁灯×8**（柱列内侧 + doorway 两侧；右中柱已断改挂残桩低处，高度/间距不对称）：
  铁架+火盆碗+**蓝紫外焰锥（0x7a8ff0）+冷白内焰锥（0xeef2ff）**+点光（0x7488e8）
  +粒子发射锚（火焰粒子淡紫/蓝紫上浮）。
- **光照**：冷环境光（0x2e3452×1.25）+ **月光平行光（0x9db4ec×2.2，唯一投影光，
  mapSize 2048，自左墙高窗泻入）** + 相机侧弱补光（0x66779e×0.35——环境/补光压低，
  窗形光池才有对比度）；幽火/吊灯点光（基准 1150/800 candela，distance 78/95——
  收敛光池只留灯周一圈）双频 sin 闪烁。**StageManager.attach 开启 shadowMap
  （PCF；PCFSoft 新版 three 已弃用），假 renderer 无 shadowMap 自动跳过**。
- **雾**：THREE.Fog(0x060a14, 215, 320) 远景没入永夜蓝黑但保留墙/窗剪影；
...
4. ~~程序化 3D 场景 + 透视相机~~ ✅ 2026-08-01（fov=24° 透视、坡面地板、火把/吊灯闪烁点光、
   ...
   瑞米改后排（前排下半身被手牌扇遮）。
5. 效果 overlay（fxAnchor 图标列；图标可先复用 richtext 徽章）。
6. 场景进阶：地板纹理/接缝、更多场景（森林/熔岩等，同 build3D 契约换皮）、前景视差层。
