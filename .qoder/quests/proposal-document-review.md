...
4. **渐进式优化**：先完成基础功能迁移，后续可以根据实际效果进行优化和调整
...
提供一个开发模式下的调试面板（Vue Devtools 插件或独立组件）：
...
- ERROR: 注册失败、动画执行异常
- WARN: 元素未注册、锚点缺失
- INFO: 动画开始/结束（生产环境可关闭）
- DEBUG: 详细的动画参数、时序信息（仅开发模式）
```
...
```
监控指标：
- 动画帧率（通过 requestAnimationFrame 计算）
- 注册表大小
- 动画队列长度（animationSequencer）
- GSAP Tween 对象数量
```
...
- 标签（tags）：`kebab-case`（如 `'card-anim'`、`'drag-blocker'`）
...
重点测试模块：
- `animator.js` 的注册表管理
- `animationHelpers.js` 的指令封装
- `interactionHandler.js` 的事件转换逻辑

#### 集成测试

关键场景：
- 完整的卡牌打出流程（从点击到动画到后端结算）
- 拖拽重排序的完整流程
- 多卡连续动画的时序正确性
